...
async def generate_videosample_from_link(
    ...
    rand_str = randstr()
    output_path = f"download/[Sample By Tg : @EliteFlixMediaInfoBot]__{unquote(filename)}"
...
async def telegram_videosample(message, client, duration):
    ...
    try:
        ...
        rand_str = randstr()
        download_path = f"download/{unquote(filename)}"
        ...
        output_path = f"download/[Sample By Tg : @EliteFlixMediaInfoBot]_{unquote(filename)}"
